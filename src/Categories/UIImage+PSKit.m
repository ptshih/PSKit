...
- (UIImage *)cropProportionalToSize:(CGSize)desiredSize withRuleOfThirds:(BOOL)withRuleOfThirds {
    ...
    if (self.size.width > self.size.height) {
        // Landscape
        newImage = [self scaleProportionalToSize:CGSizeMake(INT_MAX, maxDimension)];
    } else if (self.size.width < self.size.height) {
        // Portrait
        newImage = [self scaleProportionalToSize:CGSizeMake(maxDimension, INT_MAX)];
    } else {
        // Square
        newImage = [self scaleProportionalToSize:CGSizeMake(maxDimension, maxDimension)];
    }
    
    CGFloat leftMargin = ceil((newImage.size.width - desiredWidth) / 2);
    CGFloat topMargin = 0.0;
    if (withRuleOfThirds) {
        topMargin = ceil((newImage.size.height / 3) / 2);
    } else {
        topMargin = ceil((newImage.size.height - desiredHeight) / 2);
    }
    
    CGRect desiredRect = CGRectMake(leftMargin, topMargin, desiredWidth, desiredHeight);
    
    CGImageRef imageRef = CGImageCreateWithImageInRect([newImage CGImage], desiredRect);
