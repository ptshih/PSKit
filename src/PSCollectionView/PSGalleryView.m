...
numCols = _numCols,
galleryViewDelegate = _galleryViewDelegate,
galleryViewDataSource = _galleryViewDataSource;
...
- (void)dealloc {
    // clear delegates
    self.galleryViewDataSource = nil;
    self.galleryViewDelegate = nil;
...
- (void)removeAndAddCellsIfNecessary {
    ...
    NSInteger numViews = [self.galleryViewDataSource numberOfViewsInGalleryView:self];
    ...
    for (NSInteger i = visibleRowTop; i <= visibleRowBottom; i++) {
        ...
        if (!visibleViews) {
            ...
            for (NSInteger j = 0; j < self.numCols; j++) {
                ...
                UIView *newView = [self.galleryViewDataSource galleryView:self viewAtIndex:k];
